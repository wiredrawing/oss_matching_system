...
class BaseEmailRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($method === "POST") {
            if ($route_name === "email.registerEmail" || $route_name === "web.member.email.register") {
                $rules = [
                    "email" => [
                        "required",
                        "email:rfc",
            ...
            } else if ($route_name === "web.member.email.reissue") {
                $rules = [
                    "email" => [
                        "required",
                        "email:rfc",
                        Rule::exists("members", "email")->where(function ($query) {
                            $query
                            // 本登録完了済み
            ...
            if ($route_name === "email.checkToken" || $route_name === "web.member.create") {
                $rules = [
                    "token" => [
                        "required",
    ...
    public function messages()
    {
        return [
            ...
            "email.exists" => ":attributeの認証に失敗したか、あるいは登録されていません。",
