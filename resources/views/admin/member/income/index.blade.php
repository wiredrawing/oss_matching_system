...
<section class="wrapp">
    ...
    <div class="mypagewrap">
        <div class="mypagebox">
            ...
            <h2>現在､申請中の収入証明確認申請一覧</h2>
        </div>
        <!--box-->
        <div class="mypagebox2 pt0">
            @if ($members->count() > 0)
                {{ Form::open([
                    'url' => action('Admin\\IncomeController@index'),
                    'method' => 'GET',
                ]) }}
                <p>検索</p>
                ...
                <table>
                    ...
                    @foreach ($members as $key => $value)
                        <tr>
                            ...
                            <td>
                                {{ $value->income_image->created_at->format('Y年m月d日') }}<br>
                                {{ $value->income_image->created_at->format('H時i分s秒') }}
                            </td>
                            <td><a href="{{ action('Admin\\MemberController@detail', ['member_id' => $value->id]) }}">
                                    <button>詳細へ</button>
                                </a>
                            </td>
                        </tr>
